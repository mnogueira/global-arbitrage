"""Local paper-trading engine with optional multi-broker order routing."""
...
from global_arbitrage.execution.broker import OrderReceipt
from global_arbitrage.execution.router import BrokerRouter, CombinedBrokerSnapshot
...
class PaperStepResult:
    ...
    broker_receipts: tuple[OrderReceipt, ...] = ()
    combined_broker_snapshot: CombinedBrokerSnapshot | None = None


def calculate_unrealized_pnl(
    entry_legs: tuple[TradeLeg, ...],
    current_legs: tuple[TradeLeg, ...],
...
class PaperTrader:
    ...
    def process_observation(
        ...
        *,
        ...
        broker_receipt_list: list[OrderReceipt] = []
        action = "hold"
        reason = "no_action"
        ...
        if position is not None:
            ...
            trade_return_bps = (
                0.0
                if position.entry_notional_brl <= 0.0
                else unrealized_pnl_brl / position.entry_notional_brl * 10_000.0
            )
            ...
            if exit_reason is None:
                ...
                action = "close"
                reason = exit_reason
                close_receipts = self._route_trade(observation, open_trade=False)
                broker_receipt_list.extend(close_receipts)
                trades_path = str(
                    self.store.append_trade(
                        observation.strategy_id,
                        {
                            ...
                            "broker_receipts": _serialize_receipts(close_receipts),
                        },
                    )
                )
        ...
        if state.open_position is None and observation.should_open:
            ...
            reason = "open_signal"
            open_receipts = self._route_trade(observation, open_trade=True)
            broker_receipt_list.extend(open_receipts)
            trades_path = str(
                self.store.append_trade(
                    observation.strategy_id,
                    {
                        ...
                        "reason": "open_signal",
                        "net_edge_bps": observation.net_edge_bps,
                        "broker_receipts": _serialize_receipts(open_receipts),
                    },
                )
            )
            if self.opportunity_store is not None:
        ...
        combined_broker_snapshot = self._combined_broker_snapshot(observation)
        broker_receipts = tuple(broker_receipt_list)
        signals_path = str(
            self.store.append_signal(
                observation.strategy_id,
                {
                    ...
                    "action": action,
                    "reason": reason,
                    "broker_equity_brl": None
                    if combined_broker_snapshot is None
                    else combined_broker_snapshot.equity_brl,
                    "broker_unrealized_pnl_brl": None
                    if combined_broker_snapshot is None
                    else combined_broker_snapshot.unrealized_pnl_brl,
                    "broker_realized_pnl_brl": None
                    if combined_broker_snapshot is None
                    else combined_broker_snapshot.realized_pnl_brl,
                },
            )
        )
        ...
        return PaperStepResult(
            ...
            combined_broker_snapshot=combined_broker_snapshot,
        )
    ...
    def _route_trade(
        ...
        *,
        ...
        if self.broker_router is None:
            return ()
        return self.broker_router.execute_trade_legs(observation.trade_legs, open_trade=open_trade)

    def _combined_broker_snapshot(
        self,
        observation: StrategyObservation,
    ) -> CombinedBrokerSnapshot | None:
        if self.broker_router is None:
            return None
        fx_rate = observation.metadata.get("fx_usdbrl")
        usdbrl = None if fx_rate in {None, ""} else float(fx_rate)
        return self.broker_router.combined_account_snapshot(usdbrl=usdbrl)


def _serialize_receipts(receipts: tuple[OrderReceipt, ...]) -> str:
    return json.dumps([asdict(receipt) for receipt in receipts], sort_keys=True)
